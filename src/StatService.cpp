...
// 获取前 20 名热门书籍详细榜单
// 注意：移除了 static，增加了 Stats:: 作用域
ErrorCode Stats::getTop20Books(QList<QPair<Book, int>>& topList) {
...
// 获取前 20 名借阅达人详细榜单
ErrorCode Stats::getTop20Readers(QList<QPair<ReaderAccount, int>>& topList) {
    LoanRecordDAO loanDAO;
    AccountDAO acDAO;
    topList.clear();
...
// 按月获取读者借阅流水明细
ErrorCode Stats::getReaderHistorybyMonth(const ReaderAccount& in, const int year, const int month, QList<LoanRecord>& results) {
...
// 按年获取读者借阅流水明细
ErrorCode Stats::getReaderHistorybyYear(const ReaderAccount& in, const int year, QList<LoanRecord>& results) {
    LoanRecordDAO lrDAO;
    LoanRecord lr_tmp;
    QList<Filter> filter = { Filter::BorrowerID };
...
// 按月统计读者借阅总数
ErrorCode Stats::getReaderCountbyMonth(const ReaderAccount& in, const int year, const int month, int& result) {
    if (!(month >= 1 && month <= 12))  return ErrorCode::ILLEGAL_INPUT;
...
// 按年统计读者借阅总数
ErrorCode Stats::getReaderCountbyYear(const ReaderAccount& in, const int year, int& result) {
    LoanRecordDAO lrDAO;
    LoanRecord lr_tmp;
    QList<Filter> filter = { Filter::BorrowerID };
...
// 统计读者未归还单册数
ErrorCode Stats::getReaderCountbyNotReturned(const ReaderAccount& in, int& result) {
    LoanRecordDAO lrDAO;
    LoanRecord lr_tmp;
    QList<Filter> filter = { Filter::BorrowerID,Filter::IsReturned };
...
// 统计读者逾期未还单册数
ErrorCode Stats::getReaderCountbyOverdue(const ReaderAccount& in, int& result) {
    LoanRecordDAO lrDAO;
    LoanRecord lr_tmp;
    QList<Filter> filter = { Filter::BorrowerID,Filter::IsReturned,Filter::IsOverdue };
